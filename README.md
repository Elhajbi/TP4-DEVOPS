...
Each build produces a uniquely tagged image (`abdelkarim12/tp4:<BUILD_NUMBER>`), enabling easy rollback and version tracking.
...
|-----|------|-------------|
| **job1tp4** | Freestyle | Basic CI job — build and test only |
| **job2tp4** | Freestyle | Full CI/CD — build, test, publish, and deploy |
| **job2tp4v2** | Pipeline (inline) | Same CI/CD flow using an inline pipeline script |
| **job3tp4** | Pipeline (SCM) | Reads the `Jenkinsfile` directly from the GitHub repository |
